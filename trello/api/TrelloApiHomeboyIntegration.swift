...
extension TrelloApi {
  func markAsDone(cardId: String, completion: @escaping (Card) -> Void) {
    self.updateCard(cardId: cardId, dueComplete: true) { newCard in
      guard let doneLabel = self.board.labels.first(where: { label in label.name == "done"}) else {
        completion(newCard)
        return
      }
      
      self.addLabelsToCard(cardId: cardId, labelIds: [doneLabel.id], completion: { card in
